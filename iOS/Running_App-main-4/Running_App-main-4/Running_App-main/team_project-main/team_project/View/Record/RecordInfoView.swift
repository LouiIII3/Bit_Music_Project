...
struct RecordInfoView: View {
    ...
    @Bindable var selectedExercise: ExerciseRecordModel // 선택된 단독 데이터
//    @State var set: Int
...
//                        } //VSTACK
                        VStack {
                            Text("KG")
                                .frame(minWidth: 50, alignment: .center)
                            ForEach(Array(selectedExercise.kg.enumerated()), id: \.offset) { index, kg in
//                                TextField("0", text: $selectedExercise.kg[index])
                                TextField("0", value: $selectedExercise.kg[index], format: .number)
                                    .multilineTextAlignment(.center) // 텍스트를 가운데 정렬
                                    .frame(minWidth: 50, minHeight: 50, alignment: .center)
                                    .keyboardType(.numberPad)
                            }
                        ...
                        VStack {
                            ...
                            ForEach(Array(selectedExercise.count.enumerated()), id: \.offset) { index, count in
                                TextField("0", value: $selectedExercise.count[index], format: .number)
                                    .multilineTextAlignment(.center) // 텍스트를 가운데 정렬
                                    .frame(minWidth: 50, minHeight: 50, alignment: .center)
                                    .keyboardType(.numberPad)
                            }
        ...
        .onChange(of: iosToWatch.update) { a, b in
            print("바뀐건 알지? \(a) -> \(b)")
        }
